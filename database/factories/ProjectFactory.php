...
class ProjectFactory extends Factory
{
    public function definition(): array
    {
        ...
        if ($status === 'expired' && $this->faker->boolean(60)) {
            ...
            // LD starts the day the (effective) contract expiry passed
            $ldStartDate = $effectiveExpiry;
            $ldStatus    = $this->faker->randomElement(['active', 'active', 'terminated']);

            if ($ldStatus === 'terminated') {
                $ldEndDate = Carbon::parse($ldStartDate)->addDays($ldDaysOverdue)->format('Y-m-d');
            }
        }
        ...
        return [
            ...
            'geotagged_location' => sprintf(
                'GPS %.4f, %.4f',
                $this->faker->latitude(8.5, 14.0),
                $this->faker->longitude(120.0, 127.0)
            ),
